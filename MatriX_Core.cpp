...
void handleGame(std::string gameId, bool amIWhite)
{
    ...
    if (!pipe) { std::cerr << "[!] Pipe failed for game." << std::endl; currentMatchesCount--; return; }
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != nullptr)
    {
        ...
        std::cout << "[GAME-DATA] " << line << std::endl << std::flush;

        if (line.find("\"moves\"") != std::string::npos)
        {
            size_t moveKey = line.find("\"moves\":\"");
            size_t start = moveKey + 9;
            ...
            if (myTurn)
            {
                ...
                if (!move.empty() && move != "moves") sendMove(gameId, move);
            }
        }
    ...
    pclose(pipe);
    currentMatchesCount--;
    std::cout << "[SYSTEM] Game over. Slot released." << std::endl << std::flush;
}
...
void streamEvents()
{
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != nullptr)
    {
        ...
        std::cout << "[EVENT] " << line << std::endl << std::flush;
        ...
        {
            size_t gStart = line.find("\"id\":\"", line.find("\"game\"")) + 6;
            std::string g_id = line.substr(gStart, line.find("\"", gStart) - gStart);
...
int main()
{
    ...
    std::cout << "[DEPLOY] MatriX_Core v7.6 Online." << std::endl << std::flush;
    while (true)
    {
        streamEvents();
